...
Jpsi1, Jpsi2, and phi particles for TPS (Triple Parton Scattering) samples.

Selection criteria for TPS:
- Jpsi1 and Jpsi2: selected by pT ordering (highest and second-highest pT)
- phi: highest pT phi that comes from a gluon decay (gluon produced in parton interaction)
...
def process_file_batch(file_batch, batch_id, output_dir, max_events_per_batch=-1):
    ...
    for fname in file_batch:
        ...
        for i, event in enumerate(events):
            ...
            # TPS selection logic:
            # Jpsi1 and Jpsi2: simply take the two highest pT J/psi
            # phi: highest pT phi that comes from gluon decay
            
            selected_jpsi1 = jpsis[0]  # Highest pT J/psi
            selected_jpsi2 = jpsis[1]  # Second highest pT J/psi
            selected_phi = None

            mother_chain_cache = {}

            # Find phi from gluon decay (highest pT first)
...
def analyze_miniaod_files(input_files, output_file, max_events=-1, n_workers=1):
    ...
    if n_workers > 1:
        ...
        temp_dir = tempfile.mkdtemp(prefix="gen_corr_tps_")
...
def analyze_miniaod_files_sequential(input_files, output_file, max_events=-1):
    ...
    for fname in input_files:
        ...
        for i, event in enumerate(events):
            ...
            # Per-event cache for mother chain
            mother_chain_cache = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze MiniAOD gen particles for TPS samples')
    parser.add_argument('-i', '--input-dir', 
                        default='/eos/user/x/xcheng/learn_MC/JJP_TPS_MC_output/MINIAOD/',
