...
Selection criteria (SPS):
- Jpsi1, Jpsi2, and phi all come from the SAME parton-level interaction
- Jpsi1 pT > Jpsi2 pT (sorted by pT)
- phi: highest-pT phi that comes from a gluon originating from the parton-level interaction
- If multiple candidates exist, select the combination with highest total pT (Jpsi1 + Jpsi2 + phi)
...
def get_ancestor_set(particle, chain_cache, ancestor_cache):
    """Get set of ancestors for a particle (cached)."""
    key = id(particle)
...
def find_common_ancestor_three(p1, p2, p3, chain_cache, ancestor_cache):
    """Find if three particles share a common ancestor."""
...
def gluon_from_hard_process(gluon):
    """Check if a gluon is from the hard process (parton-level interaction)."""
    # Hard process particles typically have status 21-29 in Pythia8
    # or status 3 in older generators
    if gluon is None:
        return False
    status = gluon.status()
    # Status 21: incoming, 22: intermediate, 23: outgoing hard process
    if 21 <= status <= 29:
        return True
    if status == 3:  # older convention
        return True
    # Also check if parent is from hard process
    if gluon.numberOfMothers() > 0:
        mom = gluon.mother(0)
        if mom.status() == 21 or mom.status() == 22 or mom.status() == 23:
            return True
    return False
...
def process_file_batch(file_batch, batch_id, output_dir, max_events_per_batch=-1):
    ...
    for fname in file_batch:
        ...
        for i, event in enumerate(events):
            ...
            # SPS selection:
            # Find the combination (Jpsi1, Jpsi2, phi) where:
            # 1. All three share a common ancestor (same parton-level interaction)
            # 2. phi comes from a gluon that originated from the hard process
            # 3. Jpsi1 pT > Jpsi2 pT
            # 4. If multiple candidates, select the one with highest total pT
            
            # First, find phi candidates from gluon decay
            phi_candidates = []
            for phi_cand in phis:
                is_from_gluon, gluon = phi_from_gluon(phi_cand, mother_chain_cache)
                if is_from_gluon:
                    phi_candidates.append((phi_cand, gluon))
            ...
            best_total_pt = -1
            for idx_a, jpsi_a in enumerate(jpsis):
                for idx_b, jpsi_b in enumerate(jpsis):
                    if idx_b <= idx_a:
                        continue
                    # jpsi_a has higher pT than jpsi_b (already sorted)
                    for phi_cand, gluon in phi_candidates:
                        # Check if all three share a common ancestor
                        has_common, common_ancestors = find_common_ancestor_three(
                            jpsi_a, jpsi_b, phi_cand, mother_chain_cache, ancestor_cache
                        )
                        if has_common:
                            total_pt = jpsi_a.pt() + jpsi_b.pt() + phi_cand.pt()
                            if total_pt > best_total_pt:
...
def analyze_miniaod_files(input_files, output_file, max_events=-1, n_workers=1):
    ...
    if n_workers > 1:
        ...
        temp_dir = tempfile.mkdtemp(prefix="gen_corr_sps_")
...
def analyze_miniaod_files_sequential(input_files, output_file, max_events=-1):
    ...
    for fname in input_files:
        ...
        for i, event in enumerate(events):
            ...
            # Find the combination (Jpsi1, Jpsi2, phi) where:
            # 1. All three share a common ancestor (same parton-level interaction)
            # 2. phi comes from a gluon decay
            # 3. Jpsi1 pT > Jpsi2 pT
            # 4. If multiple candidates, select the one with highest total pT
            selected_jpsi1 = None
            selected_jpsi2 = None
            ...
            # Find the best combination with highest total pT
            best_total_pt = -1
            for idx_a, jpsi_a in enumerate(jpsis):
                for idx_b, jpsi_b in enumerate(jpsis):
                    if idx_b <= idx_a:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze MiniAOD gen particles (SPS selection)')
    parser.add_argument('-i', '--input-dir', 
                        default='/eos/user/x/xcheng/learn_MC/JJP_SPS_MC_output/MINIAOD/',
                        help='Input directory containing MiniAOD files')
    parser.add_argument('-o', '--output', 
                        default='gen_correlation_histograms_sps.root',
